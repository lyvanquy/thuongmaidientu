      ...
      <div className="container-main space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">Tất cả hợp đồng điện tử dự thảo và đã ký</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="card p-4 flex flex-col items-center justify-center text-center gap-2 border-b-4 border-indigo-400">
            <div className="w-12 h-12 rounded-xl bg-indigo-50 text-indigo-600 flex items-center justify-center">
              <FileSignature size={20} />
            </div>
            <div>
              <p className="text-xs font-semibold text-slate-500">Đang soạn</p>
              <p className="text-xl font-black text-indigo-700">{stats.draft}</p>
            </div>
          </div>
          <div className="card p-4 flex flex-col items-center justify-center text-center gap-2 border-b-4 border-amber-400">
            <div className="w-12 h-12 rounded-xl bg-amber-50 text-amber-600 flex items-center justify-center">
              <Clock size={20} />
            </div>
            <div>
              <p className="text-xs font-semibold text-slate-500">Chờ chữ ký</p>
              <p className="text-xl font-black text-amber-700">{stats.pending}</p>
            </div>
          </div>
          <div className="card p-4 flex flex-col items-center justify-center text-center gap-2 border-b-4 border-green-400">
            <div className="w-12 h-12 rounded-xl bg-green-50 text-green-600 flex items-center justify-center">
              <CheckCircle size={20} />
            </div>
            <div>
              <p className="text-xs font-semibold text-slate-500">Đã chốt / Ký</p>
              <p className="text-xl font-black text-green-700">{stats.signed}</p>
            </div>
          </div>
        </div>
          ...
          <div className="p-4 border-b border-slate-100 flex items-center gap-4 bg-slate-50">
            <div className="relative flex-1 max-w-sm">
              ...
              <input 
                ...
                className="w-full pl-9 pr-4 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 focus:outline-none"
              />
            </div>
          </div>
          ...
          <div className="overflow-x-auto">
            {isLoading ? (
              ...
              <div className="p-16 text-center text-slate-400">
                ...
                <p>Bạn không có hợp đồng B2B nào phù hợp.</p>
              </div>
            ) : (
              <table className="w-full text-sm text-left whitespace-nowrap">
                <thead className="bg-slate-50 border-b border-slate-200 text-slate-600">
                  <tr>
                    <th className="px-5 py-4 font-semibold uppercase text-[10px] tracking-wider">Mã HĐ</th>
                    ...
                    <th className="px-5 py-4 font-semibold uppercase text-[10px] tracking-wider text-right">Chi tiết</th>
                  </tr>
                </thead>
                      ...
                      <tr key={c.id} className="hover:bg-slate-50 transition-colors group">
                        ...
                        <td className="px-5 py-4">
                          ...
                          <p className="text-xs text-slate-500 mt-1">
                            {isSupplier ? 'Người mua:' : 'Nhà cung cấp:'} <span className="font-semibold text-slate-700">{partner?.name}</span>
                          </p>
                        </td>
                        ...
                        <td className="px-5 py-4 text-center">
                          <span className={`badge ${
                            ...
                            c.status === 'COMPLETED' ? 'bg-teal-100 text-teal-700' :
                        ...
                        <td className="px-5 py-4 text-right">
                          <Link 
                            href={`/contracts/${c.id}`} 
                            className="inline-flex items-center gap-1 px-3 py-1.5 border border-slate-200 text-slate-600 rounded-lg text-xs font-semibold hover:bg-white shadow-sm"
                          >
